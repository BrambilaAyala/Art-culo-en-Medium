...
aves_path = '/home/lcc_rn_02/pajaros/Dataset_Aves'
...
def create_resnet_model(num_classes):
    ...
    predictions = Dense(num_classes, activation='softmax')(x) # <-- ¡USAR num_classes de la firma!
    # Modelo completo
...
def create_data_generators():
    ...
    # Obtener lista canónica de clases desde train_directory
    ...
    if len(classes) == 0:
        raise ValueError(f"No hay subcarpetas de clase en {train_directory} -> revisa la estructura.")

    print(f"Clases detectadas en train (canónicas): {len(classes)}")
    ...
    nums = {
        'train': train_generator.num_classes,
        'valid': valid_generator.num_classes,
        'test': test_generator.num_classes
    }
    print("Número de clases por generador:", nums)
    if not (nums['train'] == nums['valid'] == nums['test']):
        raise ValueError(f"Mismatch de número de clases detectado: {nums}. "
                         "Asegúrate que las mismas subcarpetas de clases existan en train/valid/test")

    return train_generator, valid_generator, test_generator, train_generator.num_classes

    #return train_generator, valid_generator, test_generator, num_classes
...
def main():
    ...
    train_generator, valid_generator, test_generator, num_classes = create_data_generators()
